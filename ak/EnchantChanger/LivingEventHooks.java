...
public class LivingEventHooks
{
	...
	public void Flight(EntityPlayer player)
	{
		...
		if (side == Side.CLIENT) {
			...
			PacketDispatcher.sendPacketToServer(Packet_EnchantChanger.getPacketLevi(this, player));
		}
	...
	public void readPacketData(ByteArrayDataInput data, EntityPlayer player)
	{
		try
		{
			this.setModeToNBT(player, data.readBoolean());
	...
	public void writePacketData(DataOutputStream dos, EntityPlayer player)
	{
		try
		{
			dos.writeBoolean(this.getModeToNBT(player));
		} catch (Exception e)
		{
			e.printStackTrace();
		}
